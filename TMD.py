...
def fsiver_all(var,par):#共21个参数
    ...
    [beta_s, eps_s, Nq_s]=par[3]
    [beta_ub, eps_ub, Nq_ub]=par[4]
    [beta_db, eps_db, Nq_db]=par[5]
    [beta_sb, eps_sb, Nq_sb]=par[6]
    #本例暂不考虑重味夸克的siverPDF，所以，将重味夸克的参数设置为0
    par_cb=[0,0,0,0,0,0]
    #定义质子和中子的sivers参数
    flavor_p_par={2:par[1]+par[0], 1:par[2]+par[0], 3:par[3]+par[0], 4:par_cb, 5:par_cb, -2:par[4]+par[0], -1:par[5]+par[0], -3:par[6]+par[0], -4:par_cb, -5:par_cb}
    flavor_n_par={2:par[2]+par[0], 1:par[1]+par[0], 3:par[3]+par[0], 4:par_cb, 5:par_cb, -2:par[5]+par[0], -1:par[4]+par[0], -3:par[6]+par[0], -4:par_cb, -5:par_cb}
    #不同靶核的siversPDF
    fsiver_p={i:fsiver([x,b],flavor_p_par[i]) for i in flavor_p_par}
    fsiver_n={i:fsiver([x,b],flavor_n_par[i]) for i in flavor_p_par}
    fsiver_D={i:(PnD*fsiver_n[i]+PpD*fsiver_p[i])/2.0 for i in flavor_p_par}
    fsiver_3He={i:(Pn*fsiver_n[i]+Pp*2.0*fsiver_p[i])/3.0 for i in flavor_p_par}
    if target=='proton':
        return fsiver_p
    elif target=='neutron':
        return fsiver_n
...
def aut_th(var,par):#最终得到的AUT,由演化因子(evo_fac),TMDpdf(f1,fsiver),以及TMDff(d1)决定
    ...
    for i in m_target:#判断靶例子的质量
        if i==target:
            M=m_target[i]
    for i in flavor_charge:#对不同味道的积分叠加
...
def aut_inte1(b,x,Qf,z,ph,par,target,hadron,charge,flavor):
    ...
    aut1=b**2*jv(1,b*ph/z)*evo_fac([Q,b])*fsi[flavor]*ff[flavor]
    return aut1
...
'''
var=[0.1,1.0,0.4,0.2,'proton','K','0']
par=[[2.0,2.0,2.0],[6.12,5.52,0.1],[4.34,18.06,0.1],[5.15,5.42,0.1],[3.18,8.89,0.1],[4.15,17.7,0.1],[4.05,17.32,0.1]]
print(aut_th(var,par))
'''
