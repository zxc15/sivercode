...
def draw_picture():
    ...
    x, kt, mu, target, flavor=0.1, 0.001, 2., 'proton', 3
    b=np.linspace(0.001,12, 30)
    ...
    for i in b:
        #mu0=mu0=2.*math.exp(-gammaE)/i+2.
        #bstar=i/(1.+i**2./BNP**2.)**0.5
        y11=th.fourier_f1_inte(i, x, kt, mu, target, flavor)
        y1.append(y11)
    
    plt.plot(b,y1,label='y1')
    #plt.plot(b,y2,label='y2')
    
    #plt.ylim(0,2)
    plt.ylabel('u fc0+C+Pg+Pp: x=0.1, kt=0, mu=2.', fontsize=14)
...
def draw_all_fsi_f1():
    ...
    for i in fu:
        ...
        #fsi[i]=[zfh*y1[j][i] for j in range(len(kt))]
        #f1[i]=[y2[j][i] for j in range(len(kt))]
    '''
    print('b<10,mu=2.,x=.1')
    for i in fu:
        print(i,f1[i])
