...
def p1_q(var):
    ...
    uplim=1.-inte_epsabs
    c1=-a_s*L*CF*(inte(p1_q_inte1, x, uplim, allpar)-inte(p1_q_inte2, 0, uplim, allpar))
    return c1
...
def p2_q(var):
    ...
    fnp=f_np([x,b])
    c1=-a_s*L*CF*(inte(p2_q_inte, x, 1., allpar)+3.*fp*fnp)
    return c1
...
def p1_q_inte1(y, x, b, target, flavor):
    mui=mu0(b)
...
def p1_q_inte2(y, x, b, target, flavor):
    mui=mu0(b)
    fp=lha.f_1([x, mui, target])[flavor]
    fnp=f_np([x,b])
    c1=4.*fnp*fp/(1.-y)
